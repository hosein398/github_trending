...
public class Theme {
    ...
    public static int getColor(Context context,int colorId){
        if(context == null){
            return Color.parseColor("#86bc42");
        }
    ...
    public static Drawable createEditTextDrawable(Context context) {
        Resources resources = context.getResources();
        Drawable defaultDrawable = resources.getDrawable(R.drawable.search_dark).mutate();
        defaultDrawable.setColorFilter(new PorterDuffColorFilter(getColor(context,R.color.md_grey400), PorterDuff.Mode.MULTIPLY));
        Drawable pressedDrawable = resources.getDrawable(R.drawable.search_dark_activated).mutate();
        pressedDrawable.setColorFilter(new PorterDuffColorFilter(getColor(context, R.color.colorPrimary), PorterDuff.Mode.MULTIPLY));
    ...
    public static Drawable getSelectorDrawable(Context context,boolean whiteBackground) {
        if (whiteBackground) {
            if (Build.VERSION.SDK_INT >= 21) {
                Drawable maskDrawable = new ColorDrawable(0xffffffff);
                ColorStateList colorStateList = new ColorStateList(
                        new int[][]{StateSet.WILD_CARD},
                        new int[]{getColor(context,R.color.material_grey300)}
                );
                return new RippleDrawable(colorStateList, new ColorDrawable(getColor(context,R.color.md_white)), maskDrawable);
            } else {
                int color = getColor(context,R.color.material_grey300);
                StateListDrawable stateListDrawable = new StateListDrawable();
                stateListDrawable.addState(new int[]{android.R.attr.state_pressed}, new ColorDrawable(color));
                stateListDrawable.addState(new int[]{android.R.attr.state_selected}, new ColorDrawable(color));
                stateListDrawable.addState(StateSet.WILD_CARD, new ColorDrawable(getColor(context,R.color.md_white)));
                return stateListDrawable;
            }
        } else {
            return createSelectorDrawable(getColor(context,R.color.material_grey300), 2);
        }
    }
